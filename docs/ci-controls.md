...
| `.github/workflows/miswiring-scan.yml` `canonical-json` | Freezes canonical JSON byte forms: raw `json.dumps` in a MAC/hash module, or any dumps-into-hashlib/hmac flow, must use `core.json.dumps_canonical` or be baselined (`.github/scripts/check_canonical_json.py`) | Daily | Baseline in `.github/scripts/canonical_json_baseline.json` |
