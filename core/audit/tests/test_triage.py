...
class TestGeneratedCode:
    ...
    def test_detects_marker_in_file_header(self, tmp_path):
        src = tmp_path / "client.py"
        src.write_text(
            "# DO NOT EDIT - generated by openapi-generator\n"
            "def get(): pass\n"
        )
        gaps = [{"name": "f", "file": "client.py"}]
        assert "client.py" in detect_generated_files(
            gaps, target_path=tmp_path,
        )
...
class TestGeneratedCodeFromDisk:
    ...
    def test_disk_header_authoritative_over_gap_source(self, tmp_path):
        # The banner lives in the FILE header; a gap's function-body
        # source can never carry it (pre-fix the gap source was
        # preferred, so every function below the file head missed the
        # banner). The on-disk header is authoritative.
