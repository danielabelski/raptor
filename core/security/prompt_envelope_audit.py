...
_UNTRUSTED_ATTRS = frozenset({
    # Source-code-derived
    "vulnerable_code", "code", "snippet", "source", "surrounding_context",
...
_PROMPT_CONSTRUCTION_FILES = (
    ...
    # /audit auxiliary prompt flows (enveloped via core.audit._util.
...
_ALLOWLIST: tuple[AllowlistEntry, ...] = (
    # ----- core/audit/checker_synthesis.py -----
    AllowlistEntry(
        file='core/audit/checker_synthesis.py',
        func_name='synthesize_verification_rule',
        attr='rule_id',
        expr_text='{cs.rule.rule_id}',
        audit_note=(
            'f-string builds an evidence stamp (engine:synth-<id>), not '
            'prompt content. rule_id is internally generated by '
            'checker_synthesis._make_rule_id via _slugify, which clamps '
