...
| `RAPTOR_CONFIG` | `~/.config/raptor/models.json` | Path to the models config consumed by `core.llm` (`{"models": [...]}` or a bare list; `//` comments allowed). **Caveat:** `packages/exploit_feasibility` reads the same variable as the path to its *analysis-settings* JSON (its chain: explicit arg > `RAPTOR_CONFIG` > `./.raptor.json` > `~/.config/raptor/config.json`) — one name, two schemas; pointing it at models.json changes exploit-feasibility config resolution too. |
